/**
 * 服务:Memberdoctor，医患关系
 */
 appservice
    .service('MemberdoctorService', function($resource, $rootScope,$q,$http,$log,ENV) {
        $log.debug("MemberdoctorService in");
        //列表
        var list;
		//当前的对象
        var obj;
        ...
        return {
			...
			/**
             * 创建Promise版
             * @param memberdoctor 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(memberdoctor,_page) {
                var thispage=page;
                if(_page)thispage=_page;
            ...
            /**
             * 更新Promise版
             * @param memberdoctor 要被更新的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            update: function(memberdoctor,_page) {
                var thispage=page;
                if(_page)thispage=_page;
                var tmpparam=_.omit(memberdoctor, function(value, key, object) {
                    return _.isObject(value) || _.isArray(value)
                ...
                tmpparam=_.extend(tmpparam, {cmd: thispage.cmd});
                $log.debug("MemberdoctorService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
