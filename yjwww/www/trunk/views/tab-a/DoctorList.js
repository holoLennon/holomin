...
appctrl.controller('DoctorListCtrl', function($scope, $rootScope, $location,$stateParams, $log, Storage, ENV, CommonService, DoctorService,$ionicHistory) {
  ...
  //0表示易健。1表示从易健。3表示易健搜索，2表示大御医搜索过来
  ...
  var productchannelId=$stateParams.productchannelId;
    ...
    $scope.init=function(){
      if(fromto==='0'){
        $log.debug("DoctorList ctrl init aaa");
        $scope.page.orderstr = "statusDuty desc,pinyin asc";
        ...
        $scope.page.where=" productchannelFamily="+productchannelId+" or productchannelFamily like '%,"+productchannelId+"'";
        $scope.page.pageNo=1;
        $scope.page.hasNextPage=false;
        $scope.list=[];
        $scope.first();
        ...
        $log.debug("DoctorList ctrl init ");
        $scope.page.where="itypeAir=0 and statusAir=1 and name like '%"+productchannelId+"%'";
        $scope.page.pageNo=1;
        $scope.page.hasNextPage=false;
        $scope.list=[];
        $scope.first();
