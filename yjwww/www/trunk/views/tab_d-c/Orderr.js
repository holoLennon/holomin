...
appctrl.controller('OrderrCtrl', function($scope, $rootScope, $log, $timeout,
    ...
    $scope.save=function(payway){
        ...
        zspecService.memberCartToOrderr($scope.obj).then(function (data) {
            ...
            if(payway==2){
              $rootScope.$broadcast('event.NeedWxpayWindow',$scope.obj);
            }
            if(payway==3){
              $rootScope.$broadcast('event.NeedAlipayWindow',$scope.obj);
            }
          if(payway==0){
