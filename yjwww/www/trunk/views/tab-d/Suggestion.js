...
appctrl.controller('SuggestionCtrl', function($scope, $rootScope, $log, $timeout,
  ...
  $scope.save=function(){
    ...
    //个推测试
    if($scope.obj.detail=='pushtest'){
      getui.init(function(cid) {CommonService.alertm('cid='+cid);Storage.set(GETUICID, cid);});
      zspecService.pushTest(Storage.get(GETUICID),_ClientInfo.cli,5);
      CommonService.alertm('推送中');
      return;
    }
    ...
    if(($scope.obj.detail).substring(0,9)=='testwxpay'){
      var orderrId = ($scope.obj.detail).substring(9);
           WxService.config().then(function (data) {
               zspecService.wxPay(orderrId).then(function (data) {
