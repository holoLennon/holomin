/**
 * 服务:DoctorProduct，医生的药架
 */
 appservice
    .service('DoctorProductService', function($resource, $rootScope,$q,$http,$log,ENV) {
        $log.debug("DoctorProductService in");
        //列表
        var list;
		//当前的对象
        var obj;
        ...
        var aurl=ENV.api + "/DoctorProduct";
        return {
            /**
             * 初次查询
			...
			/**
             * 创建Promise版
             * @param doctorProduct 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(doctorProduct,_page) {
                var thispage=page;
                if(_page)thispage=_page;
                var tmpparam=_.omit(doctorProduct, function(value, key, object) {
                    return _.isObject(value) || _.isArray(value)
                        || key.indexOf("String") != -1 || key.indexOf("gmt") != -1
                        ;
                });
                tmpparam=_.extend(tmpparam, {cmd: thispage.cmd});
                $log.debug("DoctorProductService create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
            ...
            /**
             * 更新Promise版
             * @param doctorProduct 要被更新的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            update: function(doctorProduct,_page) {
                var thispage=page;
                if(_page)thispage=_page;
                var tmpparam=_.omit(doctorProduct, function(value, key, object) {
                    return _.isObject(value) || _.isArray(value)
                ...
                tmpparam=_.extend(tmpparam, {cmd: thispage.cmd});
                $log.debug("DoctorProductService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
