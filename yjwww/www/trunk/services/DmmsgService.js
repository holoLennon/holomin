 ...
 */
 appservice
    .service('DmmsgService', function($resource, $rootScope,$q,$http,$log,ENV) {
        $log.debug("DmmsgService in");
        //列表
        var list;
		//当前的对象
        var obj;
        ...
        return {
             ...
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(dmmsg,_page) {
                var thispage=page;
                if(_page)thispage=_page;
                var tmpparam=_.omit(dmmsg, function(value, key, object) {
                    return _.isObject(value) || _.isArray(value)
                        || key.indexOf("String") != -1 || key.indexOf("gmt") != -1
                        ;
                });
                tmpparam=_.extend(tmpparam, {cmd: thispage.cmd});
                $log.debug("DmmsgService create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
