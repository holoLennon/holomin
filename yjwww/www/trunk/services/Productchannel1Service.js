/**
 * 服务:Productchannel1，商品目录1
 */
 appservice
    .service('Productchannel1Service', function($resource, $rootScope,$q,$http,$log,ENV) {
        $log.debug("Productchannel1Service in");
        //列表
        var list;
		//当前的对象
        var obj;
        ...
        var aurl=ENV.api + "/Productchannel1";
        return {
            /**
             * 初次查询
			...
			/**
             * 创建Promise版
             * @param productchannel1 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(productchannel1,_page) {
                var thispage=page;
                if(_page)thispage=_page;
                var tmpparam=_.omit(productchannel1, function(value, key, object) {
                    return _.isObject(value) || _.isArray(value)
                        || key.indexOf("String") != -1 || key.indexOf("gmt") != -1
                        ;
                });
                tmpparam=_.extend(tmpparam, {cmd: thispage.cmd});
                $log.debug("Productchannel1Service create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
            ...
            /**
             * 更新Promise版
             * @param productchannel1 要被更新的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            update: function(productchannel1,_page) {
                var thispage=page;
                if(_page)thispage=_page;
                var tmpparam=_.omit(productchannel1, function(value, key, object) {
                    return _.isObject(value) || _.isArray(value)
                ...
                tmpparam=_.extend(tmpparam, {cmd: thispage.cmd});
                $log.debug("Productchannel1Service update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
            ...
            newObj: function() {
                return _.clone(_Productchannel1);
            }
        };
    })
