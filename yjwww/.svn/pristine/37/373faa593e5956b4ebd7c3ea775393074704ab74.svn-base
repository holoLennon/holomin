/**
 * 服务:StatiDoctorDaily，医生日统计
 */
 appservice
    .service('StatiDoctorDailyService', function($resource, $rootScope,$q,$http,$log,ENV) {
        $log.debug("StatiDoctorDailyService in");
        //列表
        var list;
		//当前的对象
        var obj;
        ...
        var aurl=ENV.api + "/StatiDoctorDaily";
        return {
            /**
             * 初次查询
			...
			/**
             * 创建Promise版
             * @param statiDoctorDaily 要被创建的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            create: function(statiDoctorDaily,_page) {
                var thispage=page;
                if(_page)thispage=_page;
                var tmpparam=_.omit(statiDoctorDaily, function(value, key, object) {
                    return _.isObject(value) || _.isArray(value)
                        || key.indexOf("String") != -1 || key.indexOf("gmt") != -1
                        ;
                });
                tmpparam=_.extend(tmpparam, {cmd: thispage.cmd});
                $log.debug("StatiDoctorDailyService create");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/create";
                $http({
            ...
            /**
             * 更新Promise版
             * @param statiDoctorDaily 要被更新的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            update: function(statiDoctorDaily,_page) {
                var thispage=page;
                if(_page)thispage=_page;
                var tmpparam=_.omit(statiDoctorDaily, function(value, key, object) {
                    return _.isObject(value) || _.isArray(value)
                ...
                tmpparam=_.extend(tmpparam, {cmd: thispage.cmd});
                $log.debug("StatiDoctorDailyService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
            ...
            newObj: function() {
                return _.clone(_StatiDoctorDaily);
            }
        };
    })
