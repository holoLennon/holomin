appctrl.controller('tabCtrl', function($scope, $rootScope, $log, $timeout,
  ...
  //显示的版本号
  $scope.h5ver=(h5ver)?h5ver:"1.0.0.0";
  ...
  $scope.$on('event.NeedLoginException', function(event,data) {
    ...
    if(data==="reg"){
      ...
      $scope.user=Storage.get(LOGINED_USER_ALL);
  ...
  $scope.nowtabs=function(nowtab){
    ...
    if('B'===nowtab){
      ...
      $scope.jumpPage('ProductTypeLinkProductListHome');
    }
  };
  /**
   * 返回屏幕一半高度
  ...
  $scope.cartDel=function(productId) {
    CommonService.confirm("是否确认要移除该商品?").then(function(res) {if(res) {
