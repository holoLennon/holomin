...
appctrl.controller('ProductCtrl', function($scope, $rootScope, $log, $timeout,
  ...
  /**
   * 如果"files_img6"这个控件发生改变，则进行上传
   */
  $scope.$watch('files_img6', function (files) {
    if (files != null) {
      if (!angular.isArray(files)) {
        $timeout(function () {
          $scope.files_img6 = files = [files];
        });
        return;
      }
      for (var i = 0; i < files.length; i++) {
  ...
  /**
   * 如果"files_imgbarcode"这个控件发生改变，则进行上传
   */
  $scope.$watch('files_imgbarcode', function (files) {
    if (files != null) {
      if (!angular.isArray(files)) {
        $timeout(function () {
          $scope.files_imgbarcode = files = [files];
        });
        return;
      }
      for (var i = 0; i < files.length; i++) {
