 ...
 * Cmschannel，内容频道详细
 */
appctrl.controller('CmschannelCtrl', function($scope, $rootScope, $log, $timeout,
                                             $ionicTabsDelegate, $ionicPopover, $ionicModal, $ionicLoading,
                                             $location, $state,
                                             $cordovaNetwork, $cordovaGoogleAnalytics,
                                             $stateParams,
                                             ENV,CommonService,CmschannelService) {
     ...
     */
    $scope.get=function(){
        $log.debug("Cmschannel ctrl get id="+id);
        if(isblank0(id)) {
            CmschannelService.newget().then(function (data) {
                $log.debug("Cmschannel ctrl newget then");
                $scope.obj=data;
            });
        }else{
    ...
    $scope.save=function(){
        ...
        $log.debug($scope.obj)
        if(isblank0(id)) {
            CmschannelService.create($scope.obj).then(function (data) {
                $log.debug("Cmschannel ctrl save then");
                $scope.obj=data;
                $location.path("/tab/Cmschannel/"+$scope.obj.id);
            });
        }else{
            CmschannelService.update($scope.obj).then(function (data) {
                $log.debug("Cmschannel ctrl update then");
                $scope.obj=data;
                $location.path("/tab/Cmschannel/"+$scope.obj.id);
            });
        }
