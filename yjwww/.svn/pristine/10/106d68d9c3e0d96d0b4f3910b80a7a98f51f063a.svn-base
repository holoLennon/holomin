...
appctrl.controller('PrescriptCreateCtrl', function($scope, $rootScope,$timeout,
   ...
   * 医生对药方产品进行添加
   */
  $scope.addPrescriptProduct=function(){
    ...
    $scope.isPaying = true;
    $scope.obj.id=$scope.act;
    $scope.obj.obj1=JSON.stringify($scope.listPrescriptProduct);
    zspecService.addPrescriptProduct($scope.obj).then(function(data) {
      if(data.msg=="ok"){
