 ...
 */
 appservice
    .service('OrderrService', function($resource, $rootScope,$q,$http,$log,ENV) {
        $log.debug("OrderrService in");
        //列表
        var list;
		//当前的对象
        var obj;
        ...
        return {
            ...
            /**
             * 更新Promise版
             * @param orderr 要被更新的对象
             * @param _page 页码对象，如果为空使用全局页码对象
             */
            update: function(orderr,_page) {
                var thispage=page;
                if(_page)thispage=_page;
                var tmpparam=_.omit(orderr, function(value, key, object) {
                    return _.isObject(value) || _.isArray(value)
                ...
                tmpparam=_.extend(tmpparam, {cmd: thispage.cmd});
                $log.debug("OrderrService update");
                $log.debug(tmpparam);
                var deferred = $q.defer();
                var url =aurl+"/update";
                $http({
