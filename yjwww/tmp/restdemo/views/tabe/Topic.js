...
appctrl.controller('TopicCtrl', function($scope, $rootScope, $log, $timeout,
     ...
     */
    $scope.get=function(){
        $log.debug("Topic ctrl get id="+id);
        if(isblank0(id)) {
            TopicService.newget().then(function (data) {
                $log.debug("Topic ctrl newget then");
                $scope.obj=data;
            });
        }else{
    ...
    $scope.save=function(){
        ...
        $log.debug($scope.obj)
        if(isblank0(id)) {
            TopicService.create($scope.obj).then(function (data) {
                $log.debug("Topic ctrl save then");
                $scope.obj=data;
                $location.path("/tab/Topic/"+$scope.obj.id);
            });
        }else{
            TopicService.update($scope.obj).then(function (data) {
                $log.debug("Topic ctrl update then");
                $scope.obj=data;
                $location.path("/tab/Topic/"+$scope.obj.id);
            });
        }
